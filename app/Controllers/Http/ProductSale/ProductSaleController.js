...
const productSaleRepository = use('App/Repository/ProductSale/productSaleRepository')
...
class ProductSaleController {
  ...
  async index ({ request, response, view }) {
    return await productSaleRepository.index()
  }
  ...
  async store ({ request, response, auth }) {
    return await productSaleRepository.store(request, auth)
  }
  ...
  /**
   * Update productsale details.
   * PUT or PATCH productsales/:id
   *
   * @param {object} ctx
   * @param {Request} ctx.request
   * @param {Response} ctx.response
